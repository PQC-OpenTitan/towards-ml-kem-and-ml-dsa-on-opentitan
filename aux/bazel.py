...
def extract_wheel(
    ...
    print(f"wheel_file: {wheel_file}, incremental: {incremental}, whl.name: {whl.name}, {type(whl.name)}")
    if incremental:
        directory = incremental_dir
    else:
        if whl.name == "None":
            print(f"Name is None {whl._path}")
            whlname = whl._path.split("-")[0]
        else:
            whlname = whl.name
        print(f"wheel name = {whlname}")
        directory = sanitise_name(whlname, prefix=repo_prefix)
        print(f"wheel dir = {directory}")
    ...
    extras_requested = extras[whlname] if whlname in extras else set()
    ...
    library_name = (
        PY_LIBRARY_LABEL if incremental else sanitise_name(whlname, repo_prefix)
    )
